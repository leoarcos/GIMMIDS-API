...
class consultasPsicologia_DAO {
    ...
    public function listarConsultasId($id, $key){
        ...
        if(hash("SHA256",$key)==$instance->key){
            ...
            $sql = "SELECT gr.* , pc.* FROM consultas_psicologia gr, pacientes pc WHERE (pc.idRegPac=gr.numid_pac AND pc.tipoidRegPac=gr.tipoid_pac) AND id_consulta= :id";
            $dbh=$instance->__connection;
    ...
    public function listarConsultasPaciente($datos ){
        ...
        if(hash("SHA256",$datos['key'])==$instance->key){
           ...
           $sql = "SELECT gr.* , usr.nombres AS nombreProfesional FROM consultas_psicologia gr, usuarios usr WHERE (gr.tipoid_pac= :tipoid_pac AND gr.numid_pac= :numid_pac)  AND usr.numid=gr.id_registra";
