...
class consultas_DAO {
    ...
    public function listarConsultas($key){
     ...
     public function listarConsultasId($id){
        ...
        if(hash("SHA256",$datos['key'])==$instance->key){
            ...
            $sql = "SELECT gr.* , pc.* FROM consultas_generales gr, pacientes pc WHERE (pc.idRegPac=gr.numid_pac AND pc.tipoidRegPac=gr.tipoid_pac) AND id_consulta= :id";
    ...
    public function listarConsultasPaciente($datos){
        ...
        if(hash("SHA256",$datos['key'])==$instance->key){
            ...
            $sql = "SELECT gr.* , usr.nombres AS nombreProfesional, usr.institucion AS instuser, usr.direccion AS diruser, usr.profesion AS profuser, usr.reg_prof AS regprousr FROM consultas_generales gr, usuarios usr WHERE (gr.tipoid_pac= :tipoid_pac AND gr.numid_pac= :numid_pac)  AND usr.numid=gr.id_registra";
            $dbh=$instance->__connection;
