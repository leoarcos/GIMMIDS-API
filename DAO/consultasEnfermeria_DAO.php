...
class consultasEnfermeria_DAO {
    ...
    public function listarConsultas($key){
        ...
        if(hash("SHA256",$key)==$instance->key){
            ...
            $sql = "SELECT gr.* , pc.nombres, pc.papellido, pc.sapellido FROM consultas_enfermeria gr, pacientes pc WHERE  pc.idRegPac=gr.numid_pac AND pc.tipoidRegPac=gr.tipoid_pac";
    ...
    public function listarConsultasId($id){
        ...
        if(hash("SHA256",$key)==$instance->key){
            ...
            $sql = "SELECT gr.* , pc.* FROM consultas_enfermeria gr, pacientes pc WHERE (pc.idRegPac=gr.numid_pac AND pc.tipoidRegPac=gr.tipoid_pac) AND id_consulta= :id";
            $dbh=$instance->__connection;
    ...
    public function listarConsultasPaciente($datos){
        ...
        if(hash("SHA256",$datos['key'])==$instance->key){
           ...
           $sql = "SELECT gr.* , usr.nombres AS nombreProfesional FROM consultas_enfermeria gr, usuarios usr WHERE (gr.tipoid_pac= :tipoid_pac AND gr.numid_pac= :numid_pac)  AND usr.numid=gr.id_registra";
    ...
    public function registerNurseConsultation($data){ 
        ...
        if(hash("SHA256",$data['key'])==$instance->key){
            ...
            if(isset($data2['horaConsulta'])){
                $data2['horaConsulta']=$data2['horaConsulta'];
            }else{
                $data2['horaConsulta']='';
            }
